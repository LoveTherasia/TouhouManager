...
public class PlaySessionDAO {
    ...
    public boolean create(PlaySession session){
        String sql = "INSERT INTO play_sessions (game_id,start_time,session_type,note) " +
                "VALUES (?,?,?,?)";

        try(Connection conn = DatabaseManager.getInstance().getConnection();
            PreparedStatement pstmt = conn.prepareStatement(sql,Statement.RETURN_GENERATED_KEYS)){

            pstmt.setInt(1,session.getGameId());
            pstmt.setTimestamp(2,Timestamp.valueOf(session.getStartTime()));
            pstmt.setString(3,session.getSessionType());
            pstmt.setString(4,session.getNote());
            ...
            System.out.println("创建游玩会话失败" + e.getMessage());
        }
    ...
    public boolean endSession(int sessionId, LocalDateTime endTime, long durationSeconds){
        ...
        try(Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            System.err.println("结束游玩会话失败" + e.getMessage());
            return false;
        }
    }
    ...
    public List<PlaySession> findByGameId(int gameId){
        ...
        try(Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            System.out.println("查询某游戏游玩记录失败" + e.getMessage());
        }
        return sessions;
    }
    ...
    public List<PlaySession> findRecent(int limit){
        ...
        try(Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            System.err.println("查询最近记录失败:" + e.getMessage());
        }
        return sessions;
    }
    ...
    public long getToTotalPlayTimeByGame(int gameId){
        ...
        try(Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            System.out.println("统计游玩时间失败" + e.getMessage());
        }
        return 0;
    }
